...
class mmWaveConfig():
    ...
    def __init__(self, runtimeConfigPath = None, mmwaveConfigPath = None):
        ...
        """
        error = False
        if not runtimeConfigPath == None:
            error =  self.parse_runtimeConfigJSON(runtimeConfigPath)
        if runtimeConfigPath == None or error:
            ...
            self.runtimeParamDict =   {
                        ...
                        'DATA_PORT'    :  "COM5",                           # UART data port designation
        ...
        error = False
        if not mmwaveConfigPath == None:
            error =  self.parse_mmWaveConfigJSON(mmwaveConfigPath)
        if mmwaveConfigPath == None or error :
            # default values stored in class (can use as a template for making your own JSONs)
    ...
    def parse_mmWaveConfigJSON(self,filePath):
        ...
        Will load default if incorrect format. Function is very rigid on Json formatting and dictionary stored in
